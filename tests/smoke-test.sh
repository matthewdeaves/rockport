...
BASE_URL="${1:?Usage: smoke-test.sh <base-url> <valid-key> [cf-client-id cf-client-secret]}"
VALID_KEY="${2:?Usage: smoke-test.sh <base-url> <valid-key> [cf-client-id cf-client-secret]}"
CF_CLIENT_ID="${3:-${CF_ACCESS_CLIENT_ID:-}}"
CF_CLIENT_SECRET="${4:-${CF_ACCESS_CLIENT_SECRET:-}}"
PASS=0
FAIL=0
...
HEALTH_BODY=$(curl -s "$BASE_URL/health" -H "Authorization: Bearer $VALID_KEY" "${CF_ARGS[@]+"${CF_ARGS[@]}"}" --max-time 10)
