...
# S3 bucket for video generation output (Luma Ray2, us-west-2).
# Bedrock async invoke requires the output bucket to be in the same region as the model endpoint.
...
#
# The us-east-1 bucket (Nova Reel, EOL 2026-09-30) was removed. The aws.us_east_1 provider alias is
# retained so Terraform can still destroy that bucket from existing state; it can be deleted after the
# first apply that removes the bucket.
...
# SSE-S3, public access blocked, DenyNonSSL, 7-day lifecycle.
